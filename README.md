...
### 확대 중에는 경로를 다시 그리지 않는다

지도는 확대할 때 그리기 컨테이너에 변환을 걸어 그 안의 경로를 통째로 키웁니다.
그래서 손대지 않은 경로는 지도와 함께 매끄럽게 커집니다. 그런데 그 사이에 경로를
다시 쓰면 좌표가 "변환 이전" 기준으로 계산되어 걸려 있는 변환과 어긋나고,
**지도만 확대되고 경로는 안 따라오는 것처럼** 보입니다.

재보면 확대 한 번에 밑선은 0번, 매 프레임 다시 그리던 지나온 길은 74번 갱신됐습니다.
지금은 확대가 도는 동안 지도 위 요소를 건드리지 않고, 끝난 뒤 한 번 맞춥니다.
그러면 둘 다 지도와 정확히 같은 배율로 움직입니다(측정: 지도 0.25배 → 경로 0.25배).
